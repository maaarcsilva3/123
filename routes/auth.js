       ...
       connection.query('SELECT product_id, product_price, product_name From products WHERE  product_id = 1; ', (err, result) => {
        ...
        if(req.session.user_type == "buyer") {
            ...
            connection.query( newQuery, (err, result) => {
                ...
                let addtoCart = {'quantity':qty, 'product_name': req.session.product_name, 'product_id': req.session.product_id, 'buyer_id':req.session.buyer_id, 'product_price': req.session.product_price};
...
//     let query= `SELECT carts.quantity, carts.product_name, products.seller_id, products.product_price
//     FROM carts
//     INNER JOIN products
...
//       } else {
//         // res.render('cartdata', { results: results });
//         console.log(results);
